...
def predictor(feat,W,b,y): 
    ...
    features = np.matrix(tweetFeatureExtractor(feat)).T  
    ...
    for l in range(0,numLevels):
        if l==0:
            z[l]=np.matrix(W[l]*features+b[l])
            activations[l]=sigmoid(z[l])
                
        else: 
                  
            z[l]=np.matrix(W[l]*activations[l-1]+b[l])
            activations[l]=sigmoid(z[l])
...
def learnPredictor(trainExamples, testExamples, featureExtractor):
    ...
    while iternum<numiters: 
        ...
        deltab=[np.matrix(np.zeros((numNeuronsPerLevel,1))) for i in range(numLevels)]
        deltab.append(np.matrix(0))

        #for all features 1,...,m
        for x,y in trainExamples:

            features = np.matrix(featureExtractor(x)).T  
